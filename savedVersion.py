"""
askAYYI Cost Calculator
Short, descriptive sections, improved naming, and robust zero-value handling.
"""
...
#  - Manages file paths for pricing and usage configurations
# ======================================
CONFIG_DIR = 'config'
...
DEFAULT_PRICING = {
    ...
    "voice_customization_cost": 0,      # Monthly voice customization option
    "setup_cost_per_assistant": 7800,   # Additional one-time cost per assistant
    "whitelabel_fee": 14550,            # One-time whitelabel fee
    "whitelabel_fee_charged": False,    # Whether whitelabel fee applies
    "whitelabel_fee_waived": False,     # If True, whitelabel fee is shown but zero
    "monthly_support_hours": 0          # Monthly tech support hours included
}

DEFAULT_USAGE_LIMITS = {
    "base_messages": 100000,          # For up to 5 assistants
    "base_minutes": 10000,            # For up to 5 assistants
    "assistant_extra_messages": 8000, # Additional messages per asst >5
    "assistant_extra_minutes": 300,   # Additional minutes per asst >5
...
def calculate_assistant_cost(num_assistants, base_per_assistant, discount_3_5, discount_6_10):
...
#  - Adjust all pricing/usage settings
# ======================================
if menu == "Admin Dashboard":
    if authenticate_admin():
        st.title("Admin Dashboard")

        st.header("Pricing Configuration (Short 10-word descriptions below)")
        with st.form("pricing_form"):
            ...
            if save_pricing_btn:
                # Defensive: ensure no negative values
                if base_monthly_cost < 0:
                    base_monthly_cost = 0
                if setup_cost < 0:
                    setup_cost = 0
                if assistant_base_cost < 0:
                    assistant_base_cost = 0
                if setup_cost_per_assistant < 0:
                    setup_cost_per_assistant = 0
                if discount_3_5 < 0:
                    discount_3_5 = 0
                if discount_6_10 < 0:
                    discount_6_10 = 0
                if voice_customization_cost < 0:
                    voice_customization_cost = 0
                if monthly_support_hours < 0:
                    monthly_support_hours = 0

                pricing['base_monthly_cost'] = base_monthly_cost
                pricing['setup_cost'] = setup_cost
                pricing['assistant_base_cost'] = assistant_base_cost
                pricing['discount_3_5'] = discount_3_5
                pricing['discount_6_10'] = discount_6_10
                pricing['voice_customization_cost'] = voice_customization_cost
                pricing['setup_cost_per_assistant'] = setup_cost_per_assistant
                pricing['pay_3month_discount'] = pay_3month_discount
                pricing['pay_12month_discount'] = pay_12month_discount
                pricing['whitelabel_fee_charged'] = whitelabel_fee_charged
                pricing['whitelabel_fee_waived'] = whitelabel_fee_waived
                pricing['monthly_support_hours'] = monthly_support_hours

                save_config(PRICING_FILE, pricing)
                st.success("Pricing config saved successfully!")

        st.header("Usage Limits Configuration (Short 10-word descriptions)")
        with st.form("usage_limits_form"):
            col3, col4 = st.columns(2)
            with col3:
                base_messages = st.number_input(
                    "Base Messages (1-5 Assts)",
                    value=usage_limits.get('base_messages', 100000),
                    step=1000,
                    format="%i",
                    help="Included for up to 5 assts"
                )
                base_minutes = st.number_input(
                    "Base Minutes (1-5 Assts)",
                    value=usage_limits.get('base_minutes', 10000),
                    step=100,
                    format="%i",
                    help="Included for up to 5 assts"
                )
            with col4:
                assistant_extra_messages = st.number_input(
                    "Extra Msgs per Asst above 5",
                    value=usage_limits.get('assistant_extra_messages', 8000),
                    step=1000,
                    format="%i",
                    help="Adds for each asst beyond 5"
                )
                assistant_extra_minutes = st.number_input(
                    "Extra Mins per Asst above 5",
                    value=usage_limits.get('assistant_extra_minutes', 300),
                    step=50,
                    format="%i",
                    help="Adds for each asst beyond 5"
                )

            st.markdown("**Overage Costs**")
            cost_per_additional_message = st.number_input(
                "Cost per Additional Message (R)",
                value=usage_limits.get('cost_per_additional_message', 0.09),
                step=0.01,
                format="%.2f"
            )
            cost_per_additional_minute = st.number_input(
                "Cost per Additional Minute (R)",
                value=usage_limits.get('cost_per_additional_minute', 3.33),
                step=0.01,
                format="%.2f"
            )

            save_usage_btn = st.form_submit_button("Save Usage Configuration")
            if save_usage_btn:
                # Defensive: ensure no negative values
                if base_messages < 0:
                    base_messages = 0
                if base_minutes < 0:
                    base_minutes = 0
                if assistant_extra_messages < 0:
                    assistant_extra_messages = 0
                if assistant_extra_minutes < 0:
                    assistant_extra_minutes = 0
                if cost_per_additional_message < 0:
                    cost_per_additional_message = 0
                if cost_per_additional_minute < 0:
                    cost_per_additional_minute = 0

                usage_limits['base_messages'] = base_messages
                usage_limits['base_minutes'] = base_minutes
                usage_limits['assistant_extra_messages'] = assistant_extra_messages
                usage_limits['assistant_extra_minutes'] = assistant_extra_minutes
                usage_limits['cost_per_additional_message'] = cost_per_additional_message
                usage_limits['cost_per_additional_minute'] = cost_per_additional_minute

                save_config(USAGE_LIMITS_FILE, usage_limits)
                st.success("Usage limits config saved successfully!")

        st.markdown("---")
        st.header("Admin Dashboard Overview")
        st.write("You can proceed to **Client Calculator**, **Call Centre Cost Calculation**, **Main Dashboard**, or **Detailed Invoice**.")
...
# ======================================
    ...
    with st.form("client_calculator_form"):
        ...
        num_assistants = st.number_input(
            label="Number of Assistants - These are the number of additional",
    ...
    if calc_btn:
        ...
        assistant_total_cost, _ = calculate_assistant_cost(
            num_assistants,
        ...
        # 2) Base monthly cost
        base_monthly_cost = max(pricing.get("base_monthly_cost", 0), 0)
        universal_setup_cost = max(pricing.get("setup_cost", 0), 0)

        # 3) Usage included
        total_included_messages, total_included_minutes = calculate_usage(num_assistants, usage_limits)

        # 4) Actual usage (rough estimate)
        #    NOTE: changed calls to minutes, messages to chat for clarity
        total_used_messages = (people_message * avg_messages_chat)
        #    If calls also produce some text messages, add them if desired:
        #    total_used_messages += (people_call * ??? ) 
        #    Currently using people_message only for text usage.
        ...
        overage_cost_msg = additional_messages * max(usage_limits.get("cost_per_additional_message", 0), 0)
        overage_cost_min = additional_minutes * max(usage_limits.get("cost_per_additional_minute", 0), 0)
        total_overage_cost = overage_cost_msg + overage_cost_min

        # 6) Voice customization monthly
        extra_voice_cost = pricing.get("voice_customization_cost", 0) if voice_customization_selected else 0

        # 7) Monthly Subtotal
        monthly_subtotal_excl_vat = base_monthly_cost + assistant_total_cost + total_overage_cost + extra_voice_cost

        # 8) Payment plan discount
        ...
        else:  # "12 Month Commitment, Pay Monthly (5% Discount)"
            discount_percentage = 5

        final_monthly_rate = monthly_subtotal_excl_vat * (1 - discount_percentage / 100.0)

        # 9) Whitelabel fee
        whitelabel_fee = 0
        if pricing.get("whitelabel_fee_charged", False):
            if pricing.get("whitelabel_fee_waived", False):
                whitelabel_fee = 0  # Show but do not charge
            else:
                whitelabel_fee = pricing.get("whitelabel_fee", 14550)

        # 10) Setup cost for all assistants
        setup_cost_per_asst = max(pricing.get("setup_cost_per_assistant", 0), 0) * num_assistants

        # 11) Plan length
        months_to_pay = 3 if "3 Month" in payment_option else 12

        # 12) Final total
        total_cost_excl_vat = (final_monthly_rate * months_to_pay) + universal_setup_cost + whitelabel_fee + setup_cost_per_asst

        # Store all final results
        st.session_state["client_assistant_total_cost"] = assistant_total_cost
        st.session_state["client_base_monthly_cost"] = base_monthly_cost
        st.session_state["client_universal_setup_cost"] = universal_setup_cost
        st.session_state["client_discount_percentage"] = discount_percentage
        st.session_state["client_monthly_subtotal_excl_vat"] = monthly_subtotal_excl_vat
        st.session_state["client_final_monthly_rate_excl_vat"] = final_monthly_rate
        st.session_state["client_total_cost_excl_vat"] = total_cost_excl_vat
        ...
        st.session_state["client_total_overage_cost"] = total_overage_cost
        ...
        st.session_state["client_whitelabel_fee"] = whitelabel_fee
        st.session_state["client_setup_cost_per_assistant"] = setup_cost_per_asst
        st.session_state["client_monthly_support_hours"] = pricing.get("monthly_support_hours", 0)

        st.session_state["client_payment_option"] = payment_option
        st.session_state["client_voice_customization_selected"] = voice_customization_selected

        st.success("Calculation done! Go to **Call Centre Cost Calculation**, **Main Dashboard**, or **Detailed Invoice** next.")
...
# ======================================
    ...
    with st.form("call_centre_form"):
        ...
        with c1:
            ...
            salary_per_agent = st.number_input("Salary/Agent (R)", min_value=0, value=default_salary_per_agent, step=500)
        with c2:
            medical_aid_per_agent = st.number_input("Medical Aid (R/agent)", min_value=0, value=default_medical_aid_per_agent, step=500)
            pension_percent = st.number_input("Pension %", min_value=0, max_value=100, value=default_pension_percent, step=1)
        with c3:
            bonus_incentive_per_agent = st.number_input("Bonus/Agent (R)", min_value=0, value=default_bonus_per_agent, step=500)
            monthly_benefits_per_agent = st.number_input("Other Benefits/Agent (R)", min_value=0, value=default_benefits_per_agent, step=100)
        with c4:
            recruitment_cost_per_agent = st.number_input("Recruitment/Agent (R)", min_value=0, value=default_recruitment_per_agent, step=500)
            training_cost_per_agent = st.number_input("Training/Agent (R)", min_value=0, value=default_training_per_agent, step=500)
            trainer_salary = st.number_input("Trainer Salary (R/mo)", min_value=0, value=default_trainer_salary, step=500)
        ...
        with tc1:
            call_center_software = st.number_input("Call Centre Software (R/mo)", min_value=0, value=default_callcenter_software, step=500)
            licensing_per_user = st.number_input("Licensing/Agent (R/mo)", min_value=0, value=default_licensing_per_user, step=50)
            crm_subscription_per_user = st.number_input("CRM/Agent (R/mo)", min_value=0, value=default_crm_sub_per_user, step=100)
        with tc2:
            hardware_cost_per_station = st.number_input("Hardware/Station (R)", min_value=0, value=default_hardware_cost_station, step=1000)
            depreciation_years = st.number_input("Hardware Depreciation (yrs)", min_value=1, value=default_depreciation_years, step=1)
            repair_maintenance_per_device = st.number_input("Repair/Device (R/yr)", min_value=0, value=default_repair_per_device, step=100)
        with tc3:
            monthly_phone_bill_per_agent = st.number_input("Telco Bill/Agent (R/mo)", min_value=0, value=default_phone_bill_per_agent, step=100)
            call_cost_per_minute = st.number_input("Cost/call minute (R)", min_value=0.0, value=default_call_cost_per_minute, step=0.05)
            internet_services = st.number_input("Internet (R/mo)", min_value=0, value=default_internet_services, step=500)
        ...
        with fc1:
            office_rent_month = st.number_input("Office Rent (R/mo)", min_value=0, value=default_office_rent_month, step=1000)
            electricity_cost_month = st.number_input("Electricity (R/mo)", min_value=0, value=default_electricity_cost_month, step=500)
            water_cost_month = st.number_input("Water (R/mo)", min_value=0, value=default_water_cost_month, step=500)
        with fc2:
            hvac_cost_month = st.number_input("HVAC (R/mo)", min_value=0, value=default_hvac_cost_month, step=500)
            stationery_month = st.number_input("Stationery (R/mo)", min_value=0, value=default_stationery_month, step=100)
            cleaning_services_month = st.number_input("Cleaning (R/mo)", min_value=0, value=default_cleaning_services_month, step=500)
        with fc3:
            office_repairs_annual = st.number_input("Office Repairs (R/yr)", min_value=0, value=default_office_repairs_annual, step=1000)
        ...
        with mc1:
            marketing_annual = st.number_input("Marketing (R/yr)", min_value=0, value=default_marketing_annual, step=2000)
            retention_campaigns_annual = st.number_input("Retention (R/yr)", min_value=0, value=default_retention_campaigns_annual, step=1000)
            engagement_events_annual = st.number_input("Events (R/yr)", min_value=0, value=default_engagement_events_annual, step=1000)
        with mc2:
            liability_insurance_annual = st.number_input("Liability Insurance (R/yr)", min_value=0, value=default_liability_insurance_annual, step=2000)
    ...
    if callcentre_calc_btn:
        ...
        monthly_total_callcentre = monthly_personnel + monthly_technology + monthly_facility + monthly_misc
        once_off_costs_callcentre = total_recruitment + total_training

        st.session_state["cc_monthly_total_callcentre"] = monthly_total_callcentre
        st.session_state["cc_once_off_costs_callcentre"] = once_off_costs_callcentre

        st.success("Calculated Call Centre costs successfully! Check Main Dashboard or Detailed Invoice next.")
...
# --------------------------------------------------
    ...
    final_monthly_rate_excl_vat = st.session_state.get("client_final_monthly_rate_excl_vat", None)
    ...
    universal_setup_cost = safe_float(st.session_state.get("client_universal_setup_cost", 0.0))
    setup_cost_for_assistants = safe_float(st.session_state.get("client_setup_cost_per_assistant", 0.0))
    ...
    whitelabel_fee = safe_float(st.session_state.get("client_whitelabel_fee", 0.0))
    ...
    monthly_subtotal_excl_vat = safe_float(st.session_state.get("client_monthly_subtotal_excl_vat", 0.0))
    final_monthly_rate_excl_vat = safe_float(st.session_state.get("client_final_monthly_rate_excl_vat", 0.0))
    total_cost_excl_vat = safe_float(st.session_state.get("client_total_cost_excl_vat", 0.0))
    ...
    total_additional_cost = safe_float(st.session_state.get("client_total_overage_cost", 0.0))
    monthly_support_hours = safe_float(st.session_state.get("client_monthly_support_hours", 0))

    monthly_total_callcentre = safe_float(st.session_state.get("cc_monthly_total_callcentre", None))
    once_off_costs_callcentre = safe_float(st.session_state.get("cc_once_off_costs_callcentre", None))

    # Custom CSS for Cards and Highlights
    st.markdown(
        """
        <style>
        /* Card styling */
    ...
    <div style="display:flex; flex-wrap:wrap; gap:2em;">
      ...
      <div class="card" style="flex:1; min-width:250px;">
        ...
        <p>R{setup_cost_for_assistants:,.2f}</p>
      </div>
    </div>
    """
        ...
        if whitelabel_fee_waived:
            st.markdown(
                ...
                <div class="card" style="max-width:400px;">
                  ...
                  <p>R14,550 (Waived, so R0 charged)</p>
                </div>
                """,
                unsafe_allow_html=True
            )
    ...
    usage_cards_html = ""  # we'll build this up
    ...
    <div class="card" style="flex:1; min-width:250px;">
        ...
        <p>R{total_additional_cost:,.2f}</p>
    </div>
    """

    # Finally, display all usage cards as a single block of HTML
    if usage_cards_html.strip():
        # Wrap them in a container with flex layout
        final_usage_html = f"""
        ...
        st.write("No usage data to display (possibly zero or not specified).")

    # If you have monthly support hours:
    if monthly_support_hours > 0:
    ...
    st.write(
        "Below is your monthly subtotal (before any discounts) and the final monthly rate."
    )
    monthly_html = f"""
    <div style="display:flex; flex-wrap:wrap; gap:2em;">
    <div class="card" style="flex:1; min-width:250px;">
        <h4 data-tooltip="Base cost + assistant cost + extras before discount.">
        Monthly Subtotal (Pre-Discount)
        </h4>
        <p>R{monthly_subtotal_excl_vat:,.2f}</p>
    </div>
    <div class="card" style="flex:1; min-width:250px;">
        <h4 data-tooltip="Actual monthly payment after applying any discount.">
        Final Monthly Cost
        </h4>
        <p>R{final_monthly_rate_excl_vat:,.2f}</p>
    </div>
    </div> 
    """
    ...
    <span class="highlighted-total">R{total_cost_excl_vat:,.2f}</span>
    </div>
    """
    ...
    # --------------------------------------------------
    if (monthly_total_callcentre is not None) and (once_off_costs_callcentre is not None):
        st.markdown("---")
        ...
        if monthly_savings > 0:
            st.success(
                f"You could save ~ R{monthly_savings:,.2f} per month "
                "using askAYYI compared to your call centre."
            )
        elif monthly_savings < 0:
            st.warning(
                f"It appears you'd spend ~ R{abs(monthly_savings):,.2f} more "
...
# ======================================
elif menu == "Quotation":  # <-- ADDED
    st.title("Quotation")
    st.write("Below is a very detailed breakdown")
    
    # We'll only proceed if the main calculation has been done
    final_monthly_rate_excl_vat = st.session_state.get("client_final_monthly_rate_excl_vat", None)
    if final_monthly_rate_excl_vat is None:
        ...
        base_monthly_cost = st.session_state.get("client_base_monthly_cost", 0)
        assistant_cost = st.session_state.get("client_assistant_total_cost", 0)
        universal_setup_cost = st.session_state.get("client_universal_setup_cost", 0)
        payment_option = st.session_state.get("client_payment_option", "Not Specified")
        discount_percentage = st.session_state.get("client_discount_percentage", 0)
        monthly_subtotal_excl_vat = st.session_state.get("client_monthly_subtotal_excl_vat", 0)
        final_monthly_rate_excl_vat = st.session_state.get("client_final_monthly_rate_excl_vat", 0)
        total_cost_excl_vat = st.session_state.get("client_total_cost_excl_vat", 0)
        ...
        total_overage_cost = st.session_state.get("client_total_overage_cost", 0)

        whitelabel_fee_charged = st.session_state.get("client_whitelabel_fee_charged", False)
        whitelabel_fee_waived = st.session_state.get("client_whitelabel_fee_waived", False)
        whitelabel_fee = st.session_state.get("client_whitelabel_fee", 0)
        setup_cost_for_assistants = st.session_state.get("client_setup_cost_per_assistant", 0)
        ...
        voice_customization_cost = pricing.get("voice_customization_cost", 0) if voice_customization_selected else 0
        ...
        if "3 Month" in payment_option:
            ...
            # Default to 3 or 12 if not recognized. 
            # Or 1 if user picked none, but let's do 3 for example's sake.
            plan_months = 3
        ...
        invoice_items.append({
            ...
            "Amount (R)": f"{base_monthly_cost:,.2f}",
            "Explanation": "A flat monthly fee that for the Master Assistant"
        })
        # 2) Assistant(s) Monthly Cost (with discount logic)
        invoice_items.append({
            "Description": f"Assistant Monthly Cost (for {num_assistants} asst.)",
            "Amount (R)": f"{assistant_cost:,.2f}",
            "Explanation": "Sum of each assistant cost, including any volume discounts."
        })
        # 3) Voice Customization if selected
        if voice_customization_selected:
            invoice_items.append({
                "Description": "Voice Customization Monthly",
                "Amount (R)": f"{voice_customization_cost:,.2f}",
        ...
        if total_overage_cost > 0:
            invoice_items.append({
                ...
                "Amount (R)": f"{total_overage_cost:,.2f}",
                "Explanation": "Charges for usage that exceeded the included messages/minutes."
            })
        # 5) Subtotal monthly
        invoice_items.append({
            "Description": "Monthly Subtotal (before plan discount)",
            "Amount (R)": f"{monthly_subtotal_excl_vat:,.2f}",
            "Explanation": f"Sum of base, assistants, voice cost: {whitelabel_fee:,.2f}, and overages before discount."
        })
        # 6) Discount
        if discount_percentage > 0:
            ...
            invoice_items.append({
                ...
                "Amount (R)": f"-{discount_amount:,.2f}",
                "Explanation": "Reduced rate from your selected payment plan."
            })
        # 7) Final monthly rate
        invoice_items.append({
            "Description": "Final Monthly askAYYI Cost (after discount)",
            "Amount (R)": f"{final_monthly_rate_excl_vat:,.2f}",
            "Explanation": "This is what you'll pay each month under the plan."
        })
        # 8) Plan length
        invoice_items.append({
            "Description": "Plan Duration (months)",
            "Amount (R)": f"{plan_months}",
            "Explanation": "Chosen plan commits you for this many months."
        })
        # 9) Setup Costs (one-time)
        invoice_items.append({
            "Description": "Universal Setup Cost (one-time)",
            "Amount (R)": f"{universal_setup_cost:,.2f}",
            "Explanation": "A one-time cost to configure the system for your business."
        })
        invoice_items.append({
            "Description": f"Setup Cost for {num_assistants} Assistants",
            "Amount (R)": f"{setup_cost_for_assistants:,.2f}",
            "Explanation": "One-time cost (setup_cost_per_assistant x number_of_assistants)."
        })
        # 10) Whitelabel Fee
        if whitelabel_fee_charged:
            if whitelabel_fee_waived:
                invoice_items.append({
                    ...
                    "Amount (R)": "0.00",
                    "Explanation": "Standard R14,550 fee is displayed but waived as R0."
                })
            else:
                invoice_items.append({
                    "Description": "Whitelabel Fee",
                    "Amount (R)": f"{whitelabel_fee:,.2f}",
                    "Explanation": "A one-time branding fee if whitelabeling is needed."
                })
        # 11) Grand total
        invoice_items.append({
            "Description": "Overall Plan Total (Excl. VAT)",
            "Amount (R)": f"{total_cost_excl_vat:,.2f}",
            "Explanation": "(Monthly Final Rate x Plan Months) + Setup(s) + Whitelabel Fee."
        })
